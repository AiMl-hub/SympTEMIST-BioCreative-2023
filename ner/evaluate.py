...
# python3 medprocner_evaluation.py -r ../data/subtask1-ner/eval_dataset.tsv -p ../results/model-multilingual-e5-base/val.tsv -t ner -o scores/
...
model_checkpoint = "./full_output/model-XLM-R_Galen"
...
validation_df = pd.read_csv("data/subtask1-ner/validation_SympTEMIST_dataset.csv")
...
create_csv_files(f"{RESULTS_PATH}/{model_name}/val.tsv", data_dict)

df = pd.DataFrame(results_dict)
df = df.T
df.columns = ["filename", "label", "start_span", "end_span", "text"]
df.to_csv(f"{RESULTS_PATH}/{model_name}/results_val.tsv", index=False, sep="\t")
