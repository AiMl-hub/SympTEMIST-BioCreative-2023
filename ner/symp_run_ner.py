...
ACCESS_TOKEN = "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"
...
class DataTrainingArguments:
    ...
    conll_loading_script: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to the datasets loading script to load local data in CONLL format"
        },
    )
    ...
    offline: bool = field(
        default=False,
        metadata={
            "help": "Determine whether to load datasets from Hugging Face or local disk."
        },
    )
    ...
    max_length: bool = field(
        default=512, metadata={"help": "Maximum sentence length for truncation. "},
    )
...
def main():
    ...
    # Set the wandb run id where this run will be logged
    run_id = wandb.util.generate_id()
    run_name = "SympTEMIST" + "_" + model_args.model_name_or_path + "_" + run_id
    wandb.init(project="SympTEMIST", name=run_name)

    # Set the wandb project where this run will be logged
    os.environ["WANDB_PROJECT"] = "SympTEMIST"
    ...
    if data_args.dataset_name is not None:
        ...
        if not data_args.offline:
            ...
            # Load the datasets from local disk
    ...
    config = AutoConfig.from_pretrained(
        ...
        use_auth_token=ACCESS_TOKEN if model_args.use_auth_token else None,
    )
    ...
    # TODO save model to HuggingFace Hub
    # training_args.push_to_hub = True
    # training_args.hub_token = ACCESS_TOKEN
    if training_args.push_to_hub:
        ...
        trainer.push_to_hub(**kwargs, private=True)
